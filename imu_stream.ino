...
void readAndSendIMU() {
  ...
  int16_t rawA[3];
    //rawAx, rawAy, rawAz
    rawA[0] = (int16_t)((buf[0] << 8) | buf[1]);
    rawA[1] = (int16_t)((buf[2] << 8) | buf[3]);
    rawA[2] = (int16_t)((buf[4] << 8) | buf[5]);
  int16_t rawG[3];
    //rawGx, rawGy, rawGz
    rawG[0] = (int16_t)((buf[8] << 8) | buf[9]);
    rawG[1] = (int16_t)((buf[10] << 8) | buf[11]);
    rawG[2] = (int16_t)((buf[12] << 8) | buf[13]);
  ...
  for(int i = 0; i < 3; i++){
    //physAX, physAY
    physA[i] = rawA[i]/ACCEL_SENSITIVITY;
  }
  float physG[3];
  for(int i = 0; i < 3; i++){
    //physGX, physGY
    physG[i] = rawG[i]/GYRO_SENSITIVITY;
  }
  ...
  // Emits one CSV line: timestamp_ms,ax,ay,az,gx,gy,gz
    ...
    Serial.print(physA[0]);
    Serial.print(',');
    
    Serial.print(physA[1]);
    Serial.print(',');
    
    Serial.print(physA[2]);
    Serial.print(',');

    Serial.print(physG[0]);
    Serial.print(',');
    
    Serial.print(physG[1]);
    Serial.print(',');

    Serial.println(physG[2]);
